...
IP_TO_MIC = {
    "192.168.137.10": [0.0,  0.0],
    "192.168.137.11": [1.0,  0.0],
    "192.168.137.12": [0.5,  0.866],
    "172.20.10.3":    [0.5,  0.866],
}
...
MIC_POSITIONS = np.array([  # ordre = KNOWN_IPS
    [0.0,  0.0],
    [1.0,  0.0],
    [0.5,  0.866],
])
...
def try_triangulate():
    global timer
    ips        = list(pending.keys())
    mic_pos    = np.array([IP_TO_MIC[ip] for ip in ips])
    timestamps = np.array([pending[ip]   for ip in ips])

    print(f"  [triangulation] ips={ips}")
    print(f"  [triangulation] timestamps={timestamps}")
    print(f"  [triangulation] mic_pos={mic_pos.tolist()}")

    # verification coherence : ecart max ne doit pas depasser propagation max possible
    dt_max = timestamps.max() - timestamps.min()
    max_dist = max(np.linalg.norm(mic_pos[i] - mic_pos[j])
                   for i in range(len(mic_pos)) for j in range(len(mic_pos)))
    dt_limit = max_dist / SPEED_OF_SOUND
    print(f"  [triangulation] dt_max={dt_max*1000:.3f} ms  limite={dt_limit*1000:.3f} ms")
    if dt_max > dt_limit * 10:
        print(f"  [triangulation] ERREUR : timestamps incohérents (ecart={dt_max:.3f}s) — horloges non synchronisees ?")
    ...
    saved = dict(pending)
    reset()
    return tdoa_triangulate(mic_pos, timestamps), saved
        ...
        with lock:
            ...
            if n == 3:
                try:
                    ...
                    print(f"  [triangulation] erreur : {e}")
